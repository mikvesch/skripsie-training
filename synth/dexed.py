...
try:
    ...
    # Stage 1 reads pre-rendered spectrograms and does not need RenderMan.
    rm = None
...
class Dexed:
    ...
    def __init__(
            self,
            plugin_path=str(DEXED_PLUGIN_PATH),
        ...
        if rm is None:
            raise RuntimeError(
                "Dexed rendering requires librenderman. It is not needed for Stage 1 training "
                "with the bundled pre-rendered dataset."
            )
        ...
        if not self.engine.load_plugin(self.plugin_path, 0):
            raise RuntimeError("Could not load Dexed plugin from {}".format(self.plugin_path))
